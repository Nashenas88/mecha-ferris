...
trait MyUnwrap {
    type Output;
    fn expect(self, message: &str) -> Self::Output;
}

impl<T> MyUnwrap for Poll<T> {
    type Output = T;
    ...
    fn expect(self, message: &str) -> T {
        match self {
            Poll::Ready(t) => t,
            Poll::Pending => panic!("unable to unwrap Poll::Pending: {}", message),
        }
    }
}
...
proptest! {
    ...
    fn f32_deser(f in any::<f32>()) {
        ...
        assert!(res.expect("should deserialize").expect("should deserialize") == f);
    }
    ...
    fn f32_tuples_deser(f1 in any::<f32>(), f2 in any::<f32>(), f3 in any::<f32>()) {
        ...
        let tup = res.expect("should deserialize").expect("should deserialize");
        assert!(tup.0 == t.0);
        assert!(tup.1 == t.1);
        assert!(tup.2 == t.2);
    }
    ...
    fn u32_deser(u in any::<u32>()) {
        ...
        assert!(res.expect("should deserialize").expect("should deserialize") == u);
    }
}
...
#[test_case::test_case(
    I2cRequest::SetAnimationFactor(1.5),
    5,
...
#[test_case::test_case(
    I2cRequest::SetMotionVector(1.5, 2.5, 3.5),
    13,
...
#[test_case::test_case(
    I2cRequest::GetCalibration{ leg: 4, joint: 5, kind: 6 },
    4,
    I2cRequestOp::Get(I2cRequestField::Calibration{ leg: 4, joint: 5, kind: 6 })
)]
#[test_case::test_case(
    I2cRequest::SetCalibration(1, 2, 3, 4.0),
    8,
    I2cRequestOp::Set(I2cRequestField::Calibration{ leg: 1, joint: 2, kind: 3 })
)]
...
fn i2c_request_deser(req: I2cRequest, expected_size: usize, expected_op: I2cRequestOp) {
    ...
    let res = <I2cRequest as Deserialize>::deserialize((op, &buf)).expect("should be complete");
    match (req, &res) {
        (
            I2cRequest::ChangeState(StateMachine::Paused),
        ...
        (
            ...
            Ok(I2cRequest::GetCalibration {
                leg: leg2,
                joint: joint2,
                kind: kind2,
            }),
        ) if leg1 == *leg2 && joint1 == *joint2 && kind1 == *kind2 => {}
        (
            I2cRequest::SetCalibration(leg1, joint1, kind1, pulse1),
            Ok(I2cRequest::SetCalibration(leg2, joint2, kind2, pulse2)),
        ) if leg1 == *leg2
            && joint1 == *joint2
            && kind1 == *kind2
            && (pulse1 - *pulse2).abs() < std::f32::EPSILON => {}
...
fn vector3_deser() {
    ...
    assert_eq!(size, 12);
    let res = <Vector3<f32> as Deserialize>::deserialize(&buf)
        .expect("should deserialize")
        .expect("should deserialize");
...
fn vector4_deser() {
    ...
    assert_eq!(size, 16);
    let res = <Vector4<f32> as Deserialize>::deserialize(&buf)
        .expect("should deserialize")
        .expect("should deserialize");
...
fn translation3_deser() {
    ...
    assert_eq!(size, 12);
    let res = <Translation3<f32> as Deserialize>::deserialize(&buf)
        .expect("should deserialize")
        .expect("should deserialize");
...
fn quaternion_deser() {
    ...
    assert_eq!(size, 16);
    let res = <UnitQuaternion<f32> as Deserialize>::deserialize(&buf)
        .expect("should deserialize")
        .expect("should deserialize");
...
fn tup4_deser() {
    ...
    assert_eq!(size, 16);
    let res = <(f32, f32, f32, f32) as Deserialize>::deserialize(&buf)
        .expect("should deserialize")
        .expect("should deserialize");
