...
pub type Targets = [Point3<f32>; 6];

const TURNING_ANGLE: f32 = core::f32::consts::FRAC_PI_4;
...
impl Animation {
    ...
    {
        match self {
            Animation::Walk(_) => self.walk_targets::<T>(walking_radius, time),
            Animation::Stand(_) => self.stand_targets::<T>(walking_radius),
            Animation::RotateCW => self.rotate_cw_targets::<T>(walking_radius, time),
            Animation::RotateCCW => self.rotate_ccw_targets::<T>(walking_radius, time),
        }
    }
...
where
    T: ExpensiveMath<f32>,
{
    Point3::new(x, y + radius * T::sin(angle), z + radius * T::cos(angle))
}
